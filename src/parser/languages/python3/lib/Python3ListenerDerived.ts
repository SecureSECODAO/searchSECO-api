...
export default class P3Listener implements Python3Listener {
    ...
    enterName(ctx: NameContext) {
        if (this.tsrs.length > 0) {
            ...
            if (this.inFunction)
                this.tsrs[this.tsrs.length-1].replace(ctx.start.tokenIndex, ctx.start.tokenIndex, "var")
    ...
    enterFunccallname(ctx: FunccallnameContext) {
        if (this.tsrs.length > 0) {
            this.tsrs[this.tsrs.length-1].replace(ctx.start.tokenIndex, ctx.start.tokenIndex, "funccall")
        } 
    }
