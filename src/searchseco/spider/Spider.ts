import { Clone, Repository, Diff, ConvenientPatch, Blame } from 'nodegit';
...
export default class Spider {
    ...
    async updateRepo(filePath: string, prevTag: string, newTag: string, prevUnchangedFiles: string[]): Promise<string[]> {
        ...
        if (prevTag) {
            ...
            const diffList = await Diff.treeToTree(this.repo, await oldCommit.getTree(), await newCommit.getTree(), null);
            const patches = await diffList.patches();
            await Promise.all(patches.map(async (patch: ConvenientPatch) => {
    ...
    */
    ...
    private async processFile(repo: Repository, file: string): Promise<CodeBlock[]> {
        if (!this.repo) {
            throw new Error('Repository not initialized.');
        }
        const blame = await Blame.file(repo, file);
